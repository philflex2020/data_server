...
static void process_message(const char* topic, const char* payload) {
    ...
    Row row = parse_payload(payload_str, *info_opt, g_cfg->project_id, *g_cfg);

    row.strings["mqtt_topic"] = topic_str;  // preserve original topic for replay
